<template>
  <section>
    <el-row>
      <el-col :span="16">
        <dopMailTitle :title="'服务区域'"></dopMailTitle>
      </el-col>
    ...
    <el-row style="margin-top:10px;">
      <el-col>
        <el-table :data="tableData?tableData.regions:[]"  border class="dop-table-list" @selection-change="handleSelectionChange" >
...
import profileFrom from '~/components/resources/regions/add-edit.vue';
export default {
  name: 'index',
  head: {
    title: '服务区域管理'
  ...
  methods: {
    ...
    getTableData() {
      ...
      return this.$axios({url: '/regions', params: this.query}).then(res=>{
        if (res.data.code === 0) {
    ...
    addSubmit: function() {
      this.$refs.addForm.$refs.form.validate(async(valid) => {
        if (valid) {
          ...
          this.$axios({method: 'post', url: '/regions', data: this.addForm.data}).then(res=>{
            if (res.data.code === 0) {
              // 消息提示
              this.$notify({
    ...
    editSubmit: function() {
      this.$refs.editForm.$refs.form.validate(async(valid) => {
        if (valid) {
          ...
          this.$axios({method: 'put', url: '/regions/' + this.editForm.data.id, data: this.editForm.data}).then(res=>{
            if (res.data.code === 0) {
              // 消息提示
              this.$notify({
                message: h('span', { class: 'dop-notification-msg'}, '服务区域修改成功!'),
                type: 'success'
              });
              this.getTableData();
            } else {
            ...
            this.$notify({
              // title: '用户创建失败!',
              // message: e.message,
              message: h('span', { class: 'dop-notification-msg'}, '服务区域修改失败!'),
              type: 'error'
            });
          });
        }
    ...
    del: function(row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        const h = this.$createElement;
        this.$axios({method: 'delete', url: '/regions/' + row.id, data: row}).then(res=>{
          if (res.data.code === 0) {
            // 消息提示
            this.$notify({
